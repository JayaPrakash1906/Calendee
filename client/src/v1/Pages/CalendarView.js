...
const CalendarView = () => {
  ...
  const isSlotAvailable = (start, end, room) => {
    ...
    // Check if the new meeting is in the past
    const now = new Date();
    if (newStart < now) {
      return false;
    }
    ...
    const minMeetingDuration = 15 * 60 * 1000;
    if ((newEnd - newStart) < minMeetingDuration) {
      return false;
    }

    // Check if room type is selected
    if (!room || !roomTypes.includes(room)) {
      return false;
    }

    // Only check meetings in the same room
    const meetingsInSameRoom = meetings.filter(meeting => meeting.room_type === room);
    
    return !meetingsInSameRoom.some(meeting => {
      const meetingStart = new Date(meeting.start);
      const meetingEnd = new Date(meeting.end);
      
      // Check if time overlaps
      return (
        (newStart >= meetingStart && newStart < meetingEnd) ||
        (newEnd > meetingStart && newEnd <= meetingEnd) ||
  ...
  const handleSelectSlot = (slotInfo) => {
    ...
    setFormData(prev => ({
      ...prev,
      start_time: formatForInput(slotInfo.start),
      end_time: formatForInput(slotInfo.end)
    }));
    setIsOpen(true);
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const minMeetingDuration = 15 * 60 * 1000;
      if ((end - start) < minMeetingDuration) {
        alert('Meetings must be at least 15 minutes long');
        return;
      }
      ...
      // Check if slot is available for this specific room
      if (!isSlotAvailable(start, end, room)) {
        alert(`The ${room} is already booked for the selected time slot. Please choose a different time or room.`);
        return;
      }
          ...
          {isLoading ? (
            ...
            <Calendar
              ...
              eventPropGetter={(event) => ({ 
                style: { 
                  backgroundColor: roomTypes.indexOf(event.room_type) % 2 === 0 ? '#4a90e2' : '#50b7a7',
                  borderRadius: '4px',
                  color: 'white',
                  border: 'none'
                } 
...
//       // Check if slot is available for this room
//       if (!isSlotAvailable(start, end, room)) {
//         alert(`The ${room} is already booked for the selected time slot. Please choose a different time or room.`);
//         return;
//       }
...
//   const handleSelectSlot = (slotInfo) => {
//     // Format dates for datetime-local input (removes timezone info)
//     const formatForInput = (date) => {
//       const d = new Date(date);
//       d.setMinutes(d.getMinutes() - d.getTimezoneOffset());
//       return d.toISOString().slice(0, 16);

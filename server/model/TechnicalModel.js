...
const UpdateTechnicalModel = async (id, type, category, urgency, location, title, description, file_name) => {
  try {
    const query = `UPDATE technical_requests SET id=$1, type=$2, category=$3, urgency=$4, location=$5, title=$6, description=$7, file_name=$8 WHERE email=$10 RETURNING *`;
    const values = [id, type, category, urgency, location, title, description, file_name];
    const result = await pool.query(query, values);
    return result.rows; // Return the updated row
  } catch (error) {
    throw error;
...
const DeleteTechnicalModel = (id) => {
  return new Promise((resolve, reject) => {
      pool.query('DELETE FROM technical_requests WHERE email = $1', [id], (err, result) => {
          if (err) {
              reject(err);
          } else {
              if (result.rowCount === 0) {  // If no rows were deleted

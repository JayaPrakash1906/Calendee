...
const BiometricAccess = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Update state to include new request
      ...
      <div className="ms-[69px] flex-grow">
        ...
        <div className="mx-10 my-5">
          ...
          <form onSubmit={handleSubmit} className="grid grid-cols-2 gap-6 bg-white p-8 rounded-lg shadow-lg mb-8">
            ...
            <div>
              <label className="block text-gray-700 font-medium mb-2">Student Mail Id</label>
              <input
                type="email"
                name="student_mail_id"
                value={formData.student_mail_id}
                onChange={handleChange}
                className="w-full p-2 border rounded-md"
                required
              />
                  ...
                  {requests.length === 0 ? (
                    ...
                    requests.map((request) => (
                      <tr key={request.id} className="border-t">
                        ...
                        <td className="px-4 py-2">{request.biometricType}</td>
                        <td className="px-4 py-2">{request.accessLocation}</td>
                        <td className="px-4 py-2">
                          <span className={`${getStatusStyle(request.status)} px-2 py-1 rounded`}>
...
//         }
//         const data = await response.json();
//         setRequests(Array.isArray(data) ? data : []); // Ensure data is an array
//       } catch (error) {
//         console.error('Error fetching requests:', error);
//         alert('Failed to fetch requests.');
//       }
...
//       const newRequest = await response.json();
//       setRequests((prevRequests) => [...(Array.isArray(prevRequests) ? prevRequests : []), newRequest]);
...
//         // }
//         //const data = await response.json();
//         // Ensure data is an array, else default to empty array
//         if (response) {
//           setRequests(response);
//         } 
//       } catch (error) {
